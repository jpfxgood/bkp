...
import fs_mod
import bkp_conf
from util import get_contents, put_contents, mail_error, mail_log
import logger
from logger import start_logger, stop_logger, wait_for_logger, log, get, stopped
...
def restart( restart_file ):
    ...
    if verbose:
        print >>sys.stderr,"Exiting backup"
...
def wait_for_workers():
    ...
    for t in worker_thread_pool:
        if t.is_alive():
            t.join()
    if verbose:
        print >>sys.stderr, "workers are done"
...
def backup_directory( path ):
    ...
    for (dirpath, dirnames, filenames) in os.walk(path):
        if verbose:
            print >>sys.stderr,"Scanning dirpath=",dirpath
        ...
        if exclude_dir:
            if verbose:
                print >>sys.stderr,"Excluding dirpath=",dirpath,"because of e=",e
            continue
        ...
        while True:
            ...
            for d in dirnames:
                if d[0] == ".":
                    if verbose:
                        print >>sys.stderr,"Deleting hidden directory =",d
                    del dirnames[didx]
                    deleted = True
                    break
                didx = didx + 1
        ...
        for f in filenames:
            ...
            if (s.st_mtime >= start_time and s.st_mtime < end_time):
                if verbose:
                    print >>sys.stderr,"Enqueuing copy work",local_path,remote_path
                work_queue.put(WorkerParams( local_path, remote_path ))
            elif not (local_path in backedup_files):
                if verbose:
                    print >>sys.stderr,"Enqueuing copy work because not in backup",local_path,remote_path
                work_queue.put(WorkerParams( local_path, remote_path ))
            else:
                if verbose:
                    print >>sys.stderr,"Not Enqueuing copy work for ", local_path, "because time is out of range and it is backed up"
...
def get_machines(base_path):
    ...
    machines = []
    ls_output = StringIO.StringIO(fs_mod.fs_ls(base_path))
    for l in ls_output:
...
def get_backups( machine_path ):
    ...
    backups = []
    try:
        ls_output = StringIO.StringIO(fs_mod.fs_ls(machine_path))
    except:
        if verbose:             
            print >>sys.stderr, "Error in get_backups probably no backups "
            print >>sys.stderr, traceback.format_exc()
        ls_output = StringIO.StringIO()
...
def get_backedup_files( machine_path ):
    ...
    for bk in backups:
        ...
        if contents:
            if verbose:
                print >>sys.stderr, "Found log file and processing it"
    
            past_config = False
            for l in StringIO.StringIO(contents):
                if not past_config:
            ...
            for l in StringIO.StringIO(fs_mod.fs_ls(bk.path,True)):
...
def list():
    ...
    for lpath, dates in backedup.items():
        for d in dates:
...
def compact():
    ...
    for m in machines:
        ...
        for b in backups:
            ...
            if verbose:
                print >>sys.stderr,"Checking backup path: ",backup_path
                
            ls_output = StringIO.StringIO(fs_mod.fs_ls(backup_path))
            empty = True
            for l in ls_output:
                m = re.match("(\s*DIR\s*)(\S*)$",l)
                if m:          
                    if verbose:
                        print >>sys.stderr,"Found directory: ",m.group(2)
            ...
            if empty:
                ...
                if verbose:
                    print >>sys.stderr,"Removed empty backup: ",backup_path
            else:
                if verbose:
                    print >>sys.stderr,"Skipped removing non-empty backup: ",backup_path
